...
def build() -> tuple[str, str, str]:
    ...
    flow = [
        ...
        ["08:10", "特徴量を作り22銘柄を学習・予測", "Ridge / ロジスティック"],
        ["08:15", "予測を保存し、公開", "Neon PostgreSQL"],
        ["08:16", "買い候補をメール送信", "Gmail SMTP"],
        ["08:17", "ダッシュボードと公開JSONを更新", "Streamlit Cloud / GitHub"],
        ["09:10", "予測・メール・DBを検証", "異常ならアラート"],
        ["15:45", "引けの実績と損益を確定", "Close update"],
        ["17:00", "その日のまとめを送信", "Daily summary"],
    ]

    parts = [
        ["データ取得", "Yahoo Finance（株価・為替・指数・商品）と米国財務省（金利）"],
        ["予測", "直近120営業日で学習。Ridgeが値幅、ロジスティックが上昇確率"],
        ["対象", "海運3・石油4・自動車5・金融5・商社5 の22銘柄"],
        ["判断の締切", "毎朝08:30 JST。以降に判明した情報は使わない"],
        ["保存先", "Neon（PostgreSQL・無料枠512MB）"],
        ["表示", "Streamlit Cloudのダッシュボードと公開JSON"],
        ["通知", "Gmail経由。朝の予測・日次まとめ・異常アラート"],
        ["実行基盤", "GitHub Actions（無料）と外部スケジューラ"],
    ]

    healthy = [
        ("先読み防止", "OK", "ok", "08:30以降は使えない。テスト9本で固定"),
        ("朝の起動", "OK", "ok", "外部トリガーで08:05に固定。実証済み"),
        ("DB容量", "OK", "ok", "209MB / 512MB。書き込みを99%削減"),
        ("二重実行", "OK", "ok", "予測もメールも冪等。二重に作られない"),
        ("表示", "OK", "ok", "復旧済み。DBが落ちても公開JSONは読める"),
    ]

    problems = [
        ("予測の的中", "未検証", "bad", "BUY8件中3件的中。桁が2つ足りない"),
        ("FX欠損", "未解決", "warn", "USD/JPY等が毎朝失敗。全22銘柄の必須指標"),
        ("朝以外の遅延", "未対応", "warn", "引け+78分、日次+76分、監視+152分"),
        ("必須指標の扱い", "検討中", "wait", "欠けても予測は出る。記録は明朝から"),
        ("トークン期限", "要対応", "wait", "2027-08-12に失効。静かに遅延へ戻る"),
    ]
    ...
    blocks = [
        section(
            ...
            "時刻はJST。すべて二重に走っても安全な作りで、外部起動と"
            "GitHubの定時実行が互いの保険になっています。",
        ),
        section(
            "構成",
            table(
        ...
        section(
            ...
            "最大の未解決は一番上です。配管はほぼ整いましたが、予測が"
            "有効かどうかは、まだ判断できる件数がありません。",
        ),
    ]

    lede = "22銘柄・08:30締切・BUY8件中3件的中・DB 209/512MB"
    footer = (
