...
def main() -> None:
    ...
    st.subheader("ページ")
    # Order matches the sidebar, which Streamlit takes from the filename
    # prefixes. 実績 sits directly under Today: the two questions asked most
    # often are "what does it say today" and "has it been right", so they
    # belong next to each other rather than nine pages apart.
    pages = (
        ("pages/1_Today.py", "Today", "当日の予測、BUY候補、cutoff・品質警告"),
        ("pages/2_History.py", "実績", "公開した予測と、その後の実績の記録"),
        ("pages/3_Stock_Detail.py", "Stock Detail", "銘柄別の予測・実績履歴"),
        ("pages/4_Factor_Analysis.py", "Factor Analysis", "標準化係数とモデル感応度"),
        ("pages/5_Sector_Analysis.py", "Sector Analysis", "業種別の横断比較"),
        ("pages/6_Backtest.py", "Backtest", "OOS評価とpaper trade"),
        ("pages/7_System_Status.py", "System Status", "run、Provider、鮮度、DB状態"),
        ("pages/8_Test.py", "テスト", "検証期間ごとの結果と、予測要素の採否判定"),
        ("pages/9_Company_Analysis.py", "Company Analysis", "企業別の係数と予測推移"),
        (
            "pages/10_Preview.py",
            "予測プレビュー",
            "次営業日の予測と、使われた指標の一覧",
