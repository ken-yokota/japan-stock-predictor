...
| `scripts/run_feature_comparison.py` | 予測要素セットの同一条件比較 | `main`, `_sign_test`, `_verdict` | 期間/学習窓/比較するset | `artifacts/feature_comparison/latest.json` | `research.walk`/models/trading | `cli compare-features` | `test_research_features.py` | なし | **採否は方向的中率と符号検定**。勝率・損益は表示のみで判定に使わない |
| `research/feature_sets.py` | 予測要素セットの定義 | `BASELINE`, `FOCUSED`, `EXTENDED`, `resolve` | set名 | `FeatureSet` | なし | dataset/walk | `test_research_features.py` | なし | `BASELINE`は比較の基準なので凍結。金利は変化率ではなく変化幅 |
| `research/dataset.py` | 1銘柄分の予測行列の組み立て | `build_indicator_frame`, `build_stock_frame` | 銘柄/期間/feature set | `IndicatorPanel`, `StockFrame` | `features.builder`/`research.*` | `research.walk` | `test_research_features.py` | なし | **1営業日ラグの唯一の実装箇所**。取得できなかった系列を`missing`で報告 |
| `research/price_features.py` | 自銘柄OHLCVの追加特徴量 | `add_extended_price_features` | 1銘柄の日足 | gap/出来高/ATR/RSI/MA乖離 | pandas/numpy | `research.dataset` | `test_research_features.py` | なし | `features/builder.py`は本番用なので触らない。`overnight_gap`はラグ必須 |
| `research/history.py` | 日足のディスクキャッシュ付き取得 | `download_daily` | symbol/期間 | 日足DataFrame | yfinance | `research.dataset` | `test_research_features.py` | なし | **当日を含む範囲はキャッシュしない**(足が未完成)。1系列=1リクエストなので期間短縮では減らない |
| `research/walk.py` | 1期間分のwalk-forward実行 | `run_window`, `require_complete_data` | 銘柄/feature set/期間 | `WindowResult` | models/trading/`research.dataset` | week-test/compare-features | `test_research_features.py` | なし | week-testと比較で**同一のループ**を使う。系列欠損時はfail closed |
