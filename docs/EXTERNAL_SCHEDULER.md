...
2026-09-20の監査では、直近5営業日の `morning_kick` はいずれも08:05:13〜14 JSTに `workflow_dispatch` され、完了しています。外部サービスの設定画面・トークン期限そのものは未確認です。外部トリガーはGitHub cronの遅延を回避できますが、GitHub API・runner待ち行列・provider・学習時間には依存し、定刻起動や配信を保証しません。

予測前に08:20 JSTの収集窓まで待機し、08:30 cutoffは遅延時も固定します。旧 `morning_kick` にはこの待機がなく、通常scheduleと入力時刻が一致していませんでした。両経路は同じ待機スクリプトを使用します。
...
成功したら「登録した」とだけ伝えてください。翌営業日以降の実際の開始・完了・配信時刻で確認します。
