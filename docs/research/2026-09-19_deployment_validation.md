...
## Scheduler consistency follow-up (2026-09-20)

The external morning kick did not share the scheduled prediction's 08:20 JST collection wait. Both now invoke `scripts.wait_for_prediction_window`; delayed starts retain the configured 08:30 cutoff. Unexpectedly early triggers fail instead of publishing after an incomplete capped wait. Holidays/backfills do not sleep toward another session. External jobs also use the scheduled job's native BLAS thread limits.

Initial full tests found two regressions: the new script used the cutoff function default, and the workflow test expected the previous inline script. The script now explicitly reads configured cutoff/timezone; the retained workflow test checks shared wiring and eight new cases exercise real clock/date boundaries. Final local validation: **1099 passed, 19 warnings, 140.05 s**; Ruff PASS; CI-scope mypy PASS for 159 application files. A broader `mypy .` also scanned intentionally untyped tests and reported 224 errors in 43 files; that broader target is not claimed passing. Existing tests were retained.

After the dependency-triggered Cloud rebuild, Stock Detail rendered and switched from INPEX to Suzuki with Feature Selection Audit expanded; Factor Analysis rendered and switched to REGRESSION without an exception. Remaining Cloud pages and mobile checks are still in progress. No new production predictions or emails were generated for these checks.
